...
{
  const float value = fconstrainf(applyDeadband(angle, 2.0f), -45.0f, 45.0f); // 2 deg deadband
...
int joystickToRcChannel(float angle)
{
  ...
  return (int)fscalef(value, -1.0f, 1.0f, -500, 500);
}
...
void loop(void)
{
  ...
  // if (digitalRead(servoPort) == HIGH) {
  //   if (currentMillis - startMillis >= period)  //test whether the period has elapsed
  //   {
  //     if (isPortOpen) {
  //       isPortOpen = false;
  //       output.channels[9] = DEFAULT_CHANNEL_VALUE - 1000;
  //     } else {
  //       isPortOpen = true;
  //       output.channels[9] = DEFAULT_CHANNEL_VALUE + 1000;
  //     }
  ...
  formattedY = event.orientation.y + offsetY;
  formattedZ = event.orientation.z + offsetZ;

  // if (digitalRead(recalButton) == HIGH) {
  //   if (currentMillis - startMillis2 >= period)  //test whether the period has elapsed
  //   {
  //     offsetY = formattedY;
  //     offsetZ = formattedZ;
  ...
  float opX = map(formattedX, -360, 360, -90, 90); // yaw
  float opY = map(formattedY, -90, 90, -90, 90);   // pitch
  float opZ = map(formattedZ, -180, 180, -90, 90); // roll
  ...
  // just touch these 3 for sensitivity
  output.channels[ROLL] = DEFAULT_CHANNEL_VALUE + angleToRcChannel(opZ);
  output.channels[PITCH] = DEFAULT_CHANNEL_VALUE + angleToRcChannel(opY/2);
  output.channels[YAW] = DEFAULT_CHANNEL_VALUE + angleToRcChannel(opX*2); 
  //nothing else
  ...
  output.channels[5] = DEFAULT_CHANNEL_VALUE + 1000;
  output.channels[6] = DEFAULT_CHANNEL_VALUE + 1000;
  if (counter_scroll > 0)
  {
    counter_scroll -= 1;
  }
